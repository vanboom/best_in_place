...
BestInPlaceEditor.prototype = {
    ...
    update: function () {
        ...
        if (this.formType in {"input": 1, "textarea": 1, "date": 1} && value === this.oldValue) {
            this.abort();
            return true;
        }
        ...
        editor.ajax({
            ...
            "dataType": BestInPlaceEditor.defaults.ajaxMethod,                        // VB
    ...
    initOptions: function () {
        ...
        // VB: allow ajax dataType
        self.ajaxDataType = self.element.data("bipAjaxDataType") || BestInPlaceEditor.defaults.ajaxDataType;                // VB
    ...
    ajax: function (options) {
        'use strict';
        this.element.trigger(jQuery.Event("best_in_place:before_send"));
        options.url = this.url;
        options.dataType = this.ajaxDataType;
// VB: allowing configurable ajax data type, see self.ajaxDataType
// VB        options.beforeSend = function (xhr) {
// VB           xhr.setRequestHeader("Accept", "application/json");
// VB      };
...
BestInPlaceEditor.forms = {
    "input": {
        activateForm: function () {
            ...
            if (this.cancelButton) {
                this.element.find("button[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.input.cancelButtonHandler);
            }
            if (!this.okButton) {
    ...
    "number": {
        activateForm: function () {
            ...
            this.element.find("input[type='number']")[0].select();
            this.element.find("form").bind('submit', {editor: this}, BestInPlaceEditor.forms.input.submitHandler);
            if (this.cancelButton) {
                this.element.find("input[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.input.cancelButtonHandler);
            }
            if (!this.okButton) {
                this.element.find("input[type='number']").bind('blur', {editor: this}, BestInPlaceEditor.forms.input.inputBlurHandler);
            }
            this.element.find("input[type='number']").bind('keyup', {editor: this}, BestInPlaceEditor.forms.input.keyupHandler);
            this.blurTimer = null;
            this.userClicked = false;
        },
    ...
    "date": {
        activateForm: function () {
            ...
            this.element.find("input[type='date']")[0].select();
            this.element.find("form").bind('submit', {editor: this}, BestInPlaceEditor.forms.input.submitHandler);
            if (this.cancelButton) {
                this.element.find("input[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.input.cancelButtonHandler);
            }
            if (!this.okButton) {
                this.element.find("input[type='date']").bind('blur', {editor: this}, BestInPlaceEditor.forms.input.inputBlurHandler);
            }
            this.element.find("input[type='date']").bind('keyup', {editor: this}, BestInPlaceEditor.forms.input.keyupHandler);
            this.blurTimer = null;
            this.userClicked = false;
        },
    ...
    "textarea": {
        activateForm: function () {
            ...
            if (this.cancelButton) {
                this.element.find("button[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.textarea.cancelButtonHandler);
            }
